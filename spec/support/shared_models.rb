...
shared_examples_for "a non-blank data" do |data, confirmation: false|
  it_should_behave_like "a required data", data, confirmation: confirmation
...
shared_examples_for "a unique data" do |data, case_sensitive = false, confirmation: false, model: nil|
  context "when #{data} already exists" do
    error_message = "has already been taken"
    before do
      obj_with_same_data = FactoryGirl.create(model)
